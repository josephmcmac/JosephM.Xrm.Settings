...
namespace JosephM.Xrm.Settings.Core
{
    public static class StringExtentions
    {
                ...
                if (index != -1)
                {
                    ...
                    if (part.Contains("-"))
                    {
                        ...
                        try
                        {
                            ...
                            if (start > end)
                                throw new Exception(
                                    string.Format("Range {0} To {1} Is Invalid. End Must Be Greater Than Start", start,
                                        end));
        ...
        public static byte[] ToByteArray(this string theString)
        {
            var bytes = new byte[theString.Length * sizeof(char)];
            Buffer.BlockCopy(theString.ToCharArray(), 0, bytes, 0, bytes.Length);
            return bytes;
        }
    }
}